document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateDashboard() {
        ...
        document.getElementById('staffAssigned').textContent = Math.round(events.reduce((a, b) => a + (b.assignedStaff ? b.assignedStaff.length : 0), 0) / (events.length * 10) * 100) + '%';
    ...
    function renderTable() {
        ...
        events.forEach((e, idx) => {
            ...
            tr.innerHTML = `
                ...
                <td>${e.venue}</td>
                <td>${e.assignedStaff ? e.assignedStaff.length : 0}</td>
                <td>${e.status}</td>
                <td>
                    <button class="actions-btn edit-btn" data-idx="${idx}" disabled>Edit</button>
                    <button class="actions-btn delete-btn" data-idx="${idx}" disabled>Delete</button>
                </td>
            `;
